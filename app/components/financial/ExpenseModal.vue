    ...
    <template #body>
      <UForm :schema="schema" :state="state" class="space-y-4" @submit="onSubmit">
        <UFormField label="Descrição" name="description">
          <UInput v-model="state.description" placeholder="Ex: Supermercado, aluguel..." class="w-full" />
        </UFormField>

        <div class="grid grid-cols-2 gap-4">
          <UFormField label="Valor (R$)" name="amount">
            <UInput v-model.number="state.amount" type="number" step="0.01" min="0" class="w-full" />
          </UFormField>
        ...
        <UFormField label="Categoria" name="categoryId">
          <USelect v-model="state.categoryId" :items="categoryOptions" value-key="value" class="w-full" />
        </UFormField>

        <UFormField name="recurring">
          <UCheckbox v-model="state.recurring" label="Despesa recorrente" />
        </UFormField>

        <UFormField v-if="state.recurring" label="Dia do mês" name="recurringDay">
          <UInput v-model.number="state.recurringDay" type="number" min="1" max="31" placeholder="Ex: 10" class="w-full" />
        </UFormField>
