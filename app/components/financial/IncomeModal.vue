    ...
    <template #body>
      <UForm :schema="schema" :state="state" class="space-y-4" @submit="onSubmit">
        <UFormField label="Fonte" name="source">
          <UInput v-model="state.source" placeholder="Ex: Salário, freelance..." class="w-full" />
        </UFormField>
        ...
        <UFormField name="recurring">
          <UCheckbox v-model="state.recurring" label="Receita recorrente" />
        </UFormField>

        <UFormField v-if="state.recurring" label="Dia do mês" name="recurringDay">
          <UInput v-model.number="state.recurringDay" type="number" min="1" max="31" placeholder="Ex: 5" class="w-full" />
        </UFormField>
