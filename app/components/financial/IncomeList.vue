    ...
    <div v-else class="space-y-2">
      ...
      >
        ...
        <div class="flex items-center gap-2">
          <span class="text-sm font-semibold text-green-500 whitespace-nowrap">
            +{{ formatCurrency(income.amount) }}
          </span>
          <UDropdownMenu
            :items="[
              [
                { label: 'Editar', icon: 'i-lucide-pencil', onSelect: () => emit('edit', income) },
                { label: 'Excluir', icon: 'i-lucide-trash-2', color: 'error' as const, onSelect: () => emit('delete', income) }
              ]
            ]"
          >
            <UButton
